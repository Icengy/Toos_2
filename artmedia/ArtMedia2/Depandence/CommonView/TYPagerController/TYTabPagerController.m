...
@interface TYTabPagerController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
{
    struct {
        ...
        unsigned int badgeAtIndex :1;
    }_tabDataSourceFlags;
    
    struct {
        unsigned int configreReusableCell :1;
        unsigned int configreReusableCellBadge :1;
...
- (void)viewWillLayoutSubviews
{
    ...
    if (!_customShadowColor) {
        _customShadowColor = [UIColor colorWithRed:230/255.f green:230/255.f blue:230/255.f alpha:1];
    }
    UIEdgeInsets contentInsets = UIEdgeInsetsMake(self.contentTopEdging + 0.5, 1, 0, 1);
    CGRect shadowPathBottom = UIEdgeInsetsInsetRect(_pagerBarView.bounds, contentInsets);
    _pagerBarView.layer.shadowPath = [UIBezierPath bezierPathWithRect:shadowPathBottom].CGPath;
    _pagerBarView.layer.shadowColor = _customShadowColor.CGColor;
    _pagerBarView.layer.shadowRadius = 0.0;
    _pagerBarView.layer.shadowOffset = CGSizeMake(0, 0);
    _pagerBarView.layer.shadowOpacity = 0.95;
}
...
#pragma mark - setter
- (void)setCustomShadowColor:(UIColor *)customShadowColor {
    _customShadowColor = customShadowColor;
}
...
- (void)setDelegate:(id<TYTabPagerControllerDelegate>)delegate
{
    ...
    _tabDelegateFlags.configreReusableCellBadge = [self.delegate respondsToSelector:@selector(pagerController:configreCell:forItemBadge:atIndexPath:)];
...
- (void)setDataSource:(id<TYPagerControllerDataSource>)dataSource
{
    ...
    _tabDataSourceFlags.badgeAtIndex = [self.dataSource respondsToSelector:@selector(pagerController:badgeAtIndex:)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_tabDataSourceFlags.badgeAtIndex) {
        NSInteger badge = [self.dataSource pagerController:self badgeAtIndex:indexPath.item];
        if (_tabDelegateFlags.configreReusableCellBadge) {
            [self.delegate pagerController:self configreCell:cell forItemBadge:badge atIndexPath:indexPath];
        }
    }
