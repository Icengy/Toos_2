...
- (IBAction)nextStep:(UIButton *)sender {
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:@"AfterGetUserCardInfo" object:@{@"name":_IDInfo.name, @"num":_IDInfo.num, @"image":_IDImage}];
	NSLog(@"%@",self.navigationController.viewControllers);
	[self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
		if ([obj isKindOfClass:[FaceRecognitionViewController class]] ||
            [obj isKindOfClass:[PhoneAuthViewController class]]) {
