...
@interface ZZPhotoPickerViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout, ZZPhotoBrowerDelegate>
...
- (UIButton *)previewBtn{
    if (!_previewBtn) {
        _previewBtn = [[UIButton alloc]initWithFrame:CGRectMake(15, 0, 50, 44)];
...
- (void)preview{
    ...
    if (self.selectArray.count == 0) {
        ...
        self.browserController.selectArray = self.selectArray;
        self.browserController.scrollIndex = [self.photoArray indexOfObject:self.selectArray.firstObject];
...
- (void)viewDidLoad {
    ...
    self.browserController.maxSelectNum = _selectNum;
}
...
- (void)browerViewController:(ZZPhotoBrowerViewController *)brower didSelectedWithIndex:(NSInteger)selectedIndex selectArray:(NSArray *)selectArray {
    ...
    [_selectArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self changeSelectButtonStateAtIndex:[self.photoArray indexOfObject:obj] withPhoto:(ZZPhoto *)obj];
    }];
