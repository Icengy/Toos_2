...
- (void)handleActionMessage:(TRTCLiveRoomIMActionType)action elem:(V2TIMElem *)elem message:(V2TIMMessage *)message json:(NSDictionary<NSString *, id> *)json {
    ...
    switch (action) {
        ...
        case TRTCLiveRoomIMActionTypeUnknown:
            ...
            NSLog(@"%@",json);
            if (json) {
                if ([self canDelegateResponseMethod:@selector(AM_trtcLiveRoom:onRecvRoomCustomMsgWithJson:fromUser:)]) {
                    [self.delegate AM_trtcLiveRoom:self onRecvRoomCustomMsgWithJson:json fromUser:liveUser];
                }
            }
            break;
        default:
