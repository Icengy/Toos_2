...
-(void)AMlogout:(void(^)(void))complain {
    [self asyncRun:^(MLVBLiveRoom *self) {
        if (self->_apiAddr == nil) {
            return;
        }
...
- (void)createRoom:(NSString *)roomID roomInfo:(NSString *)roomName completion:(void(^)(int errCode, NSString *errMsg))completion {
    [self asyncRun:^(MLVBLiveRoom *self) {
        ...
        roomInfo.roomCreator = [UserInfoManager shareManager].uid;
        self.roomInfo = roomInfo;
        self.createRoomCompletion = completion;
        
        //这里不再需要从腾讯获取推流地址，已经在外面从我们自己的服务器获取了推流地址，所以直接调completion完成就可以了
...
//        [self createGroupRoomID:roomID roomName:roomName];
    }];
}
...
/**
 ...
 2. 播放房间的混流播放地址
 */
- (void)pusherEnterRoom:(NSString *)roomID completion:(void(^)(int errCode, NSString *errMsg))completion {
    __weak __typeof(self) weakSelf = self;
    
    [self asyncRun:^(MLVBLiveRoom *self){
        self->_roomRole = RoomRoleCreator;  // 房间角色为普通观众
        self.roomInfo = [[MLVBRoomInfo alloc] init];
        self.roomInfo.roomID = roomID;
...
- (void)sendAMCustomRoomTextMsgWithModel:(AMLiveMsgModel *)model textMsg:(NSString *)textMsg completion:(void (^)(AMLiveMsgModel *model, int errCode, NSString *errMsg))completion{
    [self asyncRun:^(MLVBLiveRoom *self) {
//        [self->_msgMgr sendAMTextMsg:textMsg completion:completion];
        [self->_msgMgr sendAMTextMsgWithModel:model textMsg:textMsg completion:completion];
    }];
}
...
//                        }];
                        NSLog(@"%@",roomID);
                        [IMManager enterRoom:roomID completion:^(int errCode, NSString *errMsg) {
                                    [weakSelf asyncRun:^(MLVBLiveRoom *self) {
                                        [self sendDebugMsg:[NSString stringWithFormat:@"加入IMGroup完成: errCode[%d] errMsg[%@]", errCode, errMsg]];
                                        if (errCode == 0) {
...
- (void)doCreateRoom:(void (^)(int errCode, NSString *errMsg, NSString * roomID))completion {
    [self asyncRun:^(MLVBLiveRoom *self) {
        ...
        params[@"userID"] = self->_currentUser.userID;
        params[@"roomID"] = self.roomInfo.roomID ?: @"";
        params[@"roomInfo"] = self.roomInfo.roomInfo;
...
//        NSDictionary *params = @{@"roomID": roomID, @"userID": user.userID, @"userName": user.userName, @"userAvatar": user.userAvatar, @"pushURL": self->_pushUrl};
        ...
        params[@"userAvatar"] = user.userAvatar;
        params[@"pushURL"] = self->_pushUrl;
...
- (void)AMonRecvGroupCustomMsg:(NSDictionary *)dicData groupID:(NSString *)groupID userID:(NSString *)userID msg:(NSString *)msg userName:(NSString *)userName userAvatar:(NSString *)userAvatar{
    if (![groupID isEqualToString:self.roomInfo.roomID]) {
        return;
    }
    NSLog(@"%@",dicData);
    dispatch_async(self.delegateQueue, ^{
        if ([self.delegate respondsToSelector:@selector(AMonRecvRoomTextMsgWithModel:roomID:userID:userName:userAvatar:message:)]) {
            [self.delegate AMonRecvRoomTextMsgWithModel:[AMLiveMsgModel yy_modelWithDictionary:dicData] roomID:groupID userID:userID userName:userName userAvatar:userAvatar message:msg];
        }
    });
}
