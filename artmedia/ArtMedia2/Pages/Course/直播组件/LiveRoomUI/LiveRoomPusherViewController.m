...
- (void)onAudienceEnter:(MLVBAudienceInfo *)audienceInfo{
    NSLog(@"%@",audienceInfo);
//    LiveRoomMsgModel *model = []
//    [_msgListView appendMsg:]
    LiveRoomMsgModel *msgMode = [[LiveRoomMsgModel alloc] init];
    msgMode.type = LiveRoomMsgModeTypeSystem;
    msgMode.userMsg = [NSString stringWithFormat:@"欢迎 %@ 进入直播间",audienceInfo.userName];
...
- (void)initCustomUI{
    LiveRoomHeadView *headView = [LiveRoomHeadView share];
    self.headView = headView;
    [self.view addSubview:headView];
    
    UIButton *close = [UIButton buttonWithType:UIButtonTypeCustom];
    close.x = K_Width - 44 -18;
    close.y = 50;
    close.width = 44;
    close.height = 44;
    [close setImage:[UIImage imageNamed:@"close_living"] forState:UIControlStateNormal];
    [close addTarget:self action:@selector(closeLiving) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:close];
}
- (void)closeLiving{
    UIAlertController * alert = [UIAlertController alertControllerWithTitle:@"您要终断直播吗？" message:@"终端直播可能导致学员流失" preferredStyle:UIAlertControllerStyleAlert];
...
//    [_btnChat setImage:[UIImage imageNamed:@"mlvb_comment"] forState:UIControlStateNormal];
    [_btnChat setTitle:@"上课发发言" forState:UIControlStateNormal];
    [_btnChat sizeToFit];
