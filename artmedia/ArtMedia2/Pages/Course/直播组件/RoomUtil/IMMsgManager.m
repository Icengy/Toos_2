...
- (void)sendAMTextMsgWithModel:(AMLiveMsgModel *)model textMsg:(NSString *)textMsg completion:(void (^)(AMLiveMsgModel *model, int errCode, NSString *errMsg))completion {
    [self asyncRun:^{
        TIMCustomElem *msgHead = [[TIMCustomElem alloc] init];
//        NSDictionary *userInfo = @{@"nickName": self->_config.userName, @"headPic": self->_config.userAvatar};
//        NSDictionary *headData = @{@"cmd": CMD_CUSTOM_TEXT_MSG, @"data": [model yy_modelToJSONObject]};
        NSDictionary *headData = [model yy_modelToJSONObject];
...
//        [msg addElem:msgBody];
        ...
        if (self->_groupConversation) {
            [self->_groupConversation sendMessage:msg succ:^{
                Log(@"sendGroupTextMsg success");
                if (completion) completion(model,0, nil);
            } fail:^(int code, NSString *msg) {
                Log(@"sendGroupTextMsg failed, textMsg[%@]", textMsg);
...
- (void)onRecvGroupMsg:(V2TIMMessage *)msg {
    ...
    if (msg.elemType == V2TIM_ELEM_TYPE_CUSTOM) {
        ...
        if (dict) {
            cmd = dict[@"cmd"];
            data = dict[@"data"];
            AMLiveMsgModel *model = [AMLiveMsgModel yy_modelWithDictionary:dict];
            if (_delegate && [_delegate respondsToSelector:@selector(AMonRecvGroupCustomMsg:groupID:userID:msg:userName:userAvatar:)]) {
                [_delegate AMonRecvGroupCustomMsg:dict groupID:_groupID userID:model.userData.userId msg:model.messageBody.messageText userName:model.userData.userName userAvatar:model.userData.userHeadImg];
            }
        }
...
//            if (_delegate && [_delegate respondsToSelector:@selector(AMonRecvGroupCustomMsg:groupID:userID:cmd:msg:userName:userAvatar:)]) {
//                [_delegate AMonRecvGroupCustomMsg:dict groupID:_groupID userID:msg.sender cmd:data[@"cmd"] msg:data[@"msg"] userName:data[@"userName"] userAvatar:data[@"userAvatar"]];
//            }
        ...
        if ([elem isKindOfClass:[V2TIMTextElem class]]) {
            ...
            if ([cmd isEqualToString:CMD_CUSTOM_TEXT_MSG] && [data isKindOfClass:[NSDictionary class]]) {
                ...
                if (_delegate && [_delegate respondsToSelector:@selector(AMonRecvGroupCustomMsg:groupID:userID:msg:userName:userAvatar:)]) {
                    [_delegate AMonRecvGroupCustomMsg:userInfo groupID:_groupID userID:msg.sender msg:msgText userName:nickName userAvatar:headPic];
                }
            } else if ([cmd isEqualToString:CMD_PUSHER_CHANGE]) {
                [_delegate onMemberChange:_groupID];
            }
