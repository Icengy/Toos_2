...
- (void)updateUser:(NSString *)userId userName:(NSString *_Nullable)userName {
    self.users[userId].username = [ToolUtil isEqualToNonNullKong:userName];
}

- (void)updateUser:(NSString *)userId isOwner:(BOOL)isOwner {
    self.users[userId].isOwner = isOwner;
}
