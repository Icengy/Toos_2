...
@interface BankCardListViewController () <UIGestureRecognizerDelegate, UITableViewDelegate ,UITableViewDataSource>
@property (weak, nonatomic) IBOutlet BaseTableView *tableView;
@property (assign, nonatomic) NSInteger selectedIndex;
@end

@implementation BankCardListViewController {
    NSMutableArray <BankCardListModel *>*_dataArray;
}
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([BankCardListTableCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([BankCardListTableCell class])];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
    ...
    if (_dataArray.count) {
        self.selectedModel = _dataArray[_selectedIndex];
        if (self.delegate && [self.delegate respondsToSelector:@selector(viewController:didSelectedArtField:)]) {
            [self.delegate viewController:self didSelectedArtField:self.selectedModel];
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BankCardListTableCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([BankCardListTableCell class]) forIndexPath:indexPath];
...
- (void)loadData:(id _Nullable)sender {
    ...
    [ApiUtil postWithParent:self url:[ApiUtilHeader getBanklist] needHUD:NO params:nil success:^(NSInteger code, id  _Nullable response) {
        [self.tableView endAllFreshing];
         ...
         if (array && array.count) {
             ...
             _dataArray = [NSArray yy_modelArrayWithClass:[BankCardListModel class] json:array].mutableCopy;
         }
         ...
         if (self.selectedModel && _dataArray.count) {
             [_dataArray enumerateObjectsUsingBlock:^(BankCardListModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                 if ([obj.id isEqualToString:self.selectedModel.id]) {
